...
namespace Chef_Plus
{
    public partial class frm_cadastro_bandeira : XtraForm
    {
        ...
        private void frm_cadastro_bandeira_Load(object sender, EventArgs e)
        {

            if (valid.GetOperation() == ModifiedOperation.Edit)
            {
                ExeSql sql_load = new ExeSql("SELECT descricao FROM bandeiras WHERE id=@id");
        ...
        private void frm_cadastro_bandeira_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void frm_cadastro_bandeira_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (valid.IsEditing())
            {
                if (e.CloseReason == CloseReason.UserClosing)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        {
                            ExeSql sql_del = new ExeSql("DELETE FROM bandeiras WHERE id = @id");
        ...
        private void btn_menu_save_Click(object sender, EventArgs e)
        {
            ...
            if (textEdit1.Text != descricao && textEdit1.Text != "")
            {
                ExeSql sql_exist1 = new ExeSql("SELECT COUNT(*) FROM bandeiras WHERE descricao=@descricao");
            ...
            {
                String query = "INSERT INTO bandeiras (date_insert) VALUES";
            ...
            String query_bandeira_update = "UPDATE bandeiras SET descricao=@descricao, date_update=@date_update";
            query_bandeira_update += " WHERE id = @id";
            ExeSql cmd_update = new ExeSql(query_bandeira_update);
