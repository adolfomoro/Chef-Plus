...
namespace Chef_Plus
{
    public partial class frm_cadastro_forma_pagamento : XtraForm
    {
        ...
        public frm_cadastro_forma_pagamento()
        {
            InitializeComponent();
        ...
        private void frm_cadastro_forma_pagamento_Load(object sender, EventArgs e)
        {

            if (valid.GetOperation() == ModifiedOperation.Edit)
            {
                ExeSql sql_load = new ExeSql("SELECT nome FROM formas_pagamento WHERE id=@id");
        ...
        private void frm_cadastro_forma_pagamento_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void frm_cadastro_forma_pagamento_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (valid.IsEditing())
            {
                if (e.CloseReason == CloseReason.UserClosing)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        {
                            ExeSql sql_del = new ExeSql("DELETE FROM formas_pagamento WHERE id = @id");
        ...
        private void btn_menu_save_Click(object sender, EventArgs e)
        {
            ...
            {
                ExeSql sql_exist1 = new ExeSql("SELECT COUNT(*) FROM formas_pagamento WHERE nome=@nome");
            ...
            {
                String query = "INSERT INTO formas_pagamento (date_insert, online) VALUES";
                query += "(@date_insert, 0) RETURNING id";
            ...
            String query_forma_pagamento_update = "UPDATE formas_pagamento SET nome=@nome, date_update=@date_update";
            query_forma_pagamento_update += " WHERE id = @id";
            ExeSql cmd_update = new ExeSql(query_forma_pagamento_update);
